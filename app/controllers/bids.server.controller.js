 ...
 * Bid middleware
 */
exports.bidByUser = function(req, res, next, userId) { 
	Bid.find({user: userId}).populate('user', 'displayName').exec(function(err, bid) {
		if (err) return next(err);
		if (! bid) return next(new Error('Failed to load Bid.'));
		req.bid = bid ;
		next();
	});
};